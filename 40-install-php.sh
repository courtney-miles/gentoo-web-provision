...
echo "dev-lang/php apache2 bzip2 cli crypt ctype curl -doc exif -ftp fpm gd gdbm hash inifile iconv intl json mhash -ncurses nls pdo  posix -postgres readline -snmp sockets ssl sqlite -sysvipc threads truetype unicode wddx xslt zip zlib" >> /etc/portage/package.use/php
